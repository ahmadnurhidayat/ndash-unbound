...
The edit record function allows users to modify existing DNS records without needing to delete and recreate them.

### Capabilities

- ✅ Edit record name (hostname)
- ✅ Edit record value (IP address, CNAME target, etc.)
- ✅ Edit TTL
- ✅ Edit priority (for MX/SRV records)
- ✅ Edit type (A → AAAA, etc.)
...
await bindService.updateRecord(
  "dionipe.id",
